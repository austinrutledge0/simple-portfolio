<template>
    <div  v-if='centerCoordinates.lng !== 0 && centerCoordinates.lat !== 0'>
        ...
        <p>
            This map uses live data from the <a href='http://open-notify.org/Open-Notify-API/ISS-Location-Now/'>ISS Location Rest API</a> to continuously track and mark the location of the International Space Station above Earth.
        </p>
    <GoogleMap
        :api-key="apiKey"
    ...
    >
      ...
      <Marker  v-for="marker in trackingMarkers" :key="marker.lat" :options="{icon: DotIcon , position: marker}" />
    ...
    setup() {
        ...
        const getIssLocation = async () => {
            await injectAxios.get('http://api.open-notify.org/iss-now.json').then((response) => {
                ...
                if(satelliteMarkerOptions.value.position.lat !== 0 && satelliteMarkerOptions.value.position.lng !== 0)
                {
                    trackingMarkers.value.push(satelliteMarkerOptions.value.position);
                }

                centerCoordinates.value = {lat:Number(response.data.iss_position.latitude), lng: Number(response.data.iss_position.longitude)}
                satelliteMarkerOptions.value = {icon: SatelliteIcon, position: {lat:Number(response.data.iss_position.latitude), lng: Number(response.data.iss_position.longitude)}}
            })
        };
